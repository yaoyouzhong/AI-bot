...
sealed class WeatherMonitor
{
    ...
    static readonly string CachePath = Path.Combine(AIBotBridge.AppPaths.GetFolderPath(Environment.SpecialFolder.ApplicationData), "AI-bot", "weather-provider-cache.json");
    ...
    void RenderText(Snapshot snapshot)
    {
        ...
        {
            _lastText = key;
            _headerBitmap = PetAssetImporter.EncodeRgb565(headerBmp);
            _dateBitmap = PetAssetImporter.EncodeRgb565(dateBmp);
            _airBitmap = PetAssetImporter.EncodeRgb565(airBmp);
    ...
    internal void LoadCache()
    {
        try
        {
            var cached = JsonSerializer.Deserialize<Snapshot>(File.ReadAllText(File.Exists(CachePath) ? CachePath : Path.Combine(AIBotBridge.AppPaths.GetFolderPath(Environment.SpecialFolder.ApplicationData), "AIClockBridge", "weather-cache.json")));
