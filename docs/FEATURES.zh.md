...
[返回首页](../README.md) · [完整安装图文指南](INSTALL.zh.md) · [直接看界面](#pages)
...
屏保增加农历日期（含闰月），与公历使用同一设备本地日期，整组移动时避开底部状态提示。农历由本机/设备独立换算，屏保模式下电脑断开也可显示；新功能需要更新桥接和固件。
...
重置需要用户确认，不属于普通安装步骤。诊断过程保留供电，详情见[完整指南中的 Wi-Fi 回退](INSTALL.zh.md#optional)。
