...
此分支把安装、刷机和连接都写在下面，不需要再找其他 Mac 教程。Intel Mac 不在当前发布范围；新版图形刷机功能的 Mac 构建、首次启动、权限及硬件交互仍需分别验证。
...
此入口同样属于尚未发布的新功能，公开 v0.2.2 没有此菜单。Mac 界面为原生窗口，Windows 图示仅用于说明操作顺序；Mac 构建和设备实测仍待完成。
