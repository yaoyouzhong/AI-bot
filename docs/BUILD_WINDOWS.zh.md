...
本节生成 Windows 安装 EXE、Windows ZIP 与源码包；固件可按刷机教程单独构建上传。开发者若使用打包脚本的 `-Firmware` 选项，须确保该脚本调用的 `python` 环境已安装 PlatformIO 6.1.18；刷机教程的独立虚拟环境不会自动应用到打包脚本。
普通用户请优先按 [Windows 安装图解](INSTALL.zh.md)下载现成测试版 ZIP，无需自行编译。
...
| 登录后自动运行 | 桥接服务 → 开机启动 | 使用当前用户登录后延迟 5 秒的计划任务；旧版启动项关闭再启用一次即可迁移，移动目录后需重新设置 |
