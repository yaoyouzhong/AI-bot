...
当前源码包含菜单栏、Claude/Codex 活动状态、Open-Meteo 天气、A/H/美股、最近成功缓存、UserDefaults 非敏感设置、Keychain 配对令牌和认证 LAN `/status`。需在 macOS 13+ 验证：
...
当前 Windows 主机没有 Swift 工具链；Mac 端额度、系统监控、音乐、USB/资源、镜像和设备控制尚未完成，已有源码也没有经过 Swift 编译，因此状态保持 `platform-unverified`。
