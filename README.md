...
当前 Windows 主机没有 Swift 工具链；Mac 端 USB 小控制帧、菜单控制、带 CRC/ACK 重试的二进制传输、天气/股票/音乐中文资源、Apple Music/Spotify 封面解码、AI/音乐屏保临时唤醒、设备信息/Wi-Fi 重置，以及带许可文件门槛的桌宠导入已有源码，但尚未编译或连接设备。国产额度和镜像仍未完成；自动化权限、播放器脚本字段、封面、屏保时序和设备管理必须在真实 Mac 上验证。已有源码没有经过真实账号或真实系统指标验收，因此状态保持 `platform-unverified`。
